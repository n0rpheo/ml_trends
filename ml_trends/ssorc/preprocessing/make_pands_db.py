...
sq1 = "SELECT abstract_id, year FROM abstracts_ml WHERE entities LIKE '%machine learning%' AND annotated=1"
cursor.execute(sq1)
...
otDF.to_pickle(os.path.join(db_path, 'ml_ot.pandas'))
wordDF.to_pickle(os.path.join(db_path, 'ml_word.pandas'))
posDF.to_pickle(os.path.join(db_path, 'ml_pos.pandas'))
lemmaDF.to_pickle(os.path.join(db_path, 'ml_lemma.pandas'))
yearDF.to_pickle(os.path.join(db_path, 'ml_year.pandas'))
